...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    const loadToast = toast.loading("Verifying Node Identity...");
      ...
      <div className="w-full max-w-md bg-white/[0.02] border border-white/10 p-10 rounded-[2.5rem] backdrop-blur-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input
                type="email"
                required
                className="w-full bg-white/5 border border-white/10 p-4 pl-12 rounded-2xl text-white outline-none focus:border-yellow-500 transition"
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-yellow-500 hover:bg-yellow-400 text-black font-black py-4 rounded-2xl transition flex items-center justify-center gap-3 italic uppercase tracking-tighter"
          >
            {loading ? <Loader2 className="animate-spin" /> : <>Request Node Cipher <ArrowRight size={18} /></>}
          </button>
        </form>
      </div>
    </div>
