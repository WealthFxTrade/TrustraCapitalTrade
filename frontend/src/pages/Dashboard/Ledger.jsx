...
import {
  LayoutDashboard, History, ArrowDownLeft, ArrowUpRight, RefreshCw,
  CheckCircle2, Clock, XCircle, Loader2, ShieldCheck, LogOut, Menu
} from 'lucide-react';
...
export default function Ledger() {
  ...
  const fetchLedger = useCallback(async (showLoader = true) => {
    ...
    try {
      // Matches the endpoint in your api.js registry
      const res = await api.get(API_ENDPOINTS.USER.TRANSACTIONS);
      
      // Handle the array response from getRecentTransactions controller
  ...
  const formatCurrency = (amount, currency = 'EUR') => {
    return new Intl.NumberFormat('de-DE', {
      style: 'currency',
      currency: currency.toUpperCase() === 'BTC' ? 'BTC' : 'EUR',
      minimumFractionDigits: currency.toUpperCase() === 'BTC' ? 8 : 2
    }).format(amount || 0);
  };
  ...
  const getStatusBadge = (status) => {
    ...
    switch(s) {
      case 'completed':
      case 'confirmed':
        return <span className={`${baseClass} bg-emerald-500/10 border-emerald-500/30 text-emerald-400`}><CheckCircle2 size={12} /> Confirmed</span>;
      case 'pending':
        return <span className={`${baseClass} bg-amber-500/10 border-amber-500/30 text-amber-400`}><Clock size={12} className="animate-pulse" /> Pending</span>;
      default:
        return <span className={`${baseClass} bg-rose-500/10 border-rose-500/30 text-rose-400`}><XCircle size={12} /> Failed</span>;
    }
  };
    ...
    <div className="flex min-h-screen bg-[#020408] text-white overflow-hidden">
      ...
      <aside className="hidden lg:flex w-80 bg-[#0a0c10] border-r border-white/5 p-8 flex-col h-screen">
        ...
        <nav className="flex-1 space-y-2">
          <button onClick={() => navigate('/dashboard')} className="flex items-center gap-4 px-6 py-4 rounded-2xl text-gray-400 hover:bg-white/5 hover:text-white transition-all w-full text-left">
