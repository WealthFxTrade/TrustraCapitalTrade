...
import {
  ...
  AlertCircle, Key, LogOut, Loader2
} from 'lucide-react';
...
export default function Settings() {
  ...
  const handleToggle2FA = async () => {
    try {
      ...
      toast.success(`2FA ${enable2FA ? 'disabled' : 'enabled'} (demo mode)`);
    } catch (err) {
      ...
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-3 space-y-2">
            {[
              ...
              <button
                ...
                className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl transition-all group focus:outline-none focus:ring-2 focus:ring-yellow-500/50 ${
                  activeTab === tab.id ? 'bg-white/10 text-white' : 'hover:bg-white/5 text-gray-400'
                }`}
                aria-label={`Go to ${tab.label} settings`}
              >
                <tab.icon size={18} className={`transition-colors ${activeTab === tab.id ? 'text-yellow-500' : 'text-gray-500 group-hover:text-yellow-500'}`} />
                <span className={`text-[10px] font-black uppercase tracking-widest ${activeTab === tab.id ? 'text-white' : 'text-gray-400 group-hover:text-white'}`}>
                  {tab.label}
                </span>
                <ChevronRight size={16} className={`ml-auto transition-opacity ${activeTab === tab.id ? 'opacity-100 text-yellow-500' : 'opacity-0 group-hover:opacity-100'}`} />
              </button>
            ))}
          </div>
          ...
          <div className="lg:col-span-9 space-y-8">
            {/* Profile Panel */}
            {activeTab === 'profile' && (
              <section className="bg-[#0a0c10] border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl transition-all animate-fadeIn">
                ...
                <form onSubmit={handleUpdateProfile} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <select
                      ...
                      onChange={handleProfileChange}
                      className="w-full bg-[#0a0c10] border border-white/5 rounded-2xl p-5 text-sm font-bold text-white outline-none focus:border-yellow-500/30 transition-all appearance-none"
                    >
                      <option className="bg-[#0a0c10]">English (UK)</option>
                      <option className="bg-[#0a0c10]">English (US)</option>
                      <option className="bg-[#0a0c10]">German (CH)</option>
                      <option className="bg-[#0a0c10]">French (FR)</option>
                      <option className="bg-[#0a0c10]">Spanish (ES)</option>
                    </select>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <select
                      ...
                      className="w-full bg-[#0a0c10] border border-white/5 rounded-2xl p-5 text-sm font-bold text-white outline-none focus:border-yellow-500/30 transition-all appearance-none"
                    >
                      <option className="bg-[#0a0c10]">UTC +1 (Zurich)</option>
                      <option className="bg-[#0a0c10]">UTC +0 (London)</option>
                      <option className="bg-[#0a0c10]">UTC -5 (New York)</option>
                      <option className="bg-[#0a0c10]">UTC +8 (Singapore)</option>
                    </select>
                  </div>
            ...
            {/* Security Panel */}
            {activeTab === 'security' && (
              <section className="bg-[#0a0c10] border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl relative overflow-hidden transition-all animate-fadeIn">
            ...
            {activeTab === 'notifications' && (
              <section className="bg-[#0a0c10] border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl transition-all animate-fadeIn">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-yellow-500/60 mb-8 flex items-center gap-3">
                  <Bell size={16} /> Notification Metrics
                </h3>
                <p className="text-sm text-gray-400 font-medium">Real-time ledger alerts, balance updates, and transaction dispatch signals configurations can be configured here. (Feature coming soon)</p>
              </section>
            )}
            ...
            {activeTab === 'privacy' && (
              <section className="bg-[#0a0c10] border border-white/5 rounded-[3rem] p-8 md:p-12 shadow-2xl transition-all animate-fadeIn">
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-yellow-500/60 mb-8 flex items-center gap-3">
                  <Shield size={16} /> Data & Session Audits
                </h3>
                <p className="text-sm text-gray-400 font-medium">Manage visibility access permissions, tracking data streams, and localized node historical session logs. (Feature coming soon)</p>
              </section>
            )}
