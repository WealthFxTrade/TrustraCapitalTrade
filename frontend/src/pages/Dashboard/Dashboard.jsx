...
export default function Dashboard() {
  const { user } = useAuth();
  // Added useContext import and destructuring
  ...
  }, [user, fetchBTCPrice, fetchStats, mounted]);
  ...
  if (contextLoading && !stats) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#05070a]">
        ...
        <p className="mt-4 text-[10px] font-black uppercase tracking-[0.3em] text-yellow-600 animate-pulse">
          Securing Trustra Node...
        </p>
      </div>
    );
  }
  ...
  const displayStats = {
    ...
    dailyROI: user?.dailyRoiRate && user?.investedAmount ? user.investedAmount * user.dailyRoiRate : 0,
    ...stats,
  };
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-300 selection:bg-yellow-500/30">
      ...
      <main className="px-6 lg:px-20 py-12 space-y-12 max-w-7xl mx-auto animate-fade-in">
        {/* Market & Identity Row */}
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard 
            ...
            highlight 
          />
          <StatCard title="Active Deployments" value={displayStats.activeNodes} />
          <StatCard 
