...
export default function Dashboard() {
  ...
  const [users, setUsers] = useState([]); // Admin view only
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // If admin, fetch the global user list
        if (adminStatus) {
      ...
      <div className="min-h-screen bg-[#020617] flex items-center justify-center p-6">
        <div className="text-center max-w-md">
          ...
          <button 
            ...
            className="bg-white text-black px-8 py-3 rounded-xl font-bold uppercase text-[10px] tracking-widest hover:bg-yellow-500 transition-colors"
          >
            Retry Connection
          </button>
        </div>
    ...
    <div className="min-h-screen bg-[#020617] text-white p-4 md:p-10 space-y-8 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 text-[10px] uppercase tracking-[0.4em] mt-1">Investor: {user.fullName || user.email}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-[#0a0f1e] p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden group">
          ...
          <h3 className="text-4xl font-black italic">€{(user.totalBalance || 0).toLocaleString('de-DE', { minimumFractionDigits: 2 })}</h3>
        </div>
        ...
        <div className="bg-[#0a0f1e] p-8 rounded-[2.5rem] border border-white/5 relative overflow-hidden group">
          ...
          <h3 className="text-3xl font-black italic uppercase tracking-tighter text-cyan-400">{user.activePlan || 'Standard Node'}</h3>
        </div>
      </div>
