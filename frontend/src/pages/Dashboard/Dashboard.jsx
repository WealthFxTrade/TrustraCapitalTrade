...
import api from '@/api/api';                                                            import VaultSection from '@/pages/Dashboard/VaultSection';
...
const LiveBitcoinPrice = () => {
  ...
  const fetchPrice = async () => {
    ...
    } catch (err) {                                                                           console.error('BTC price fetch failed:', err);                                          setError('Price unavailable');
    } finally {
      setLoading(false);
    }
  };                                                                                    
  useEffect(() => {
    fetchPrice();
    const interval = setInterval(fetchPrice, 60000); // 60 seconds                          return () => clearInterval(interval);
  }, []);
  ...
  return (                                                                                  <div className="bg-gray-800/60 border border-gray-700 rounded-xl px-5 py-3 text-sm backdrop-blur-sm">
      <span className="text-gray-400 mr-2">BTC/EUR:</span>
      <span className="font-bold">{price || '—'}</span>
      <span className={`ml-2 font-medium ${isPositive ? 'text-green-400' : 'text-red-400'}`}>                                                                                           {isPositive ? '+' : ''}{change24h || '—'}%
      </span>                                                                               </div>
  );                                                                                    };
                                                                                        // ── Stat Card Component ──
const StatCard = ({ label, value, icon: Icon, color = 'text-white', prefix = '€', suffix = '' }) => (
  ...
  >
    <div className="flex items-center justify-between mb-4">
      ...
      <Icon size={24} className={`${color} opacity-90`} />                                  </div>
...
export default function Dashboard() {
    ...
    try {
      ...
      if (historyRes.data.history?.length > 0) {
        ...
        const mock = Array.from({ length: 30 }, (_, i) => ({
          date: new Date(now - (29 - i) * 86400000).toLocaleDateString('en-US', {                   month: 'short',
            day: 'numeric',
          }),
  ...
  useEffect(() => {
    if (!user?._id) return;                                                             
    const socketUrl = import.meta.env.VITE_API_URL || 'http://localhost:10000';             const socket = io(socketUrl, {
    ...
    socket.on('portfolioUpdate', (update) => {
      setPortfolio((prev) => ({                                                                 ...prev,
        totalValue: update.totalValue ?? prev.totalValue,                                       pnl: update.pnl ?? prev.pnl,
  ...
  useEffect(() => {                                                                         fetchDashboardData();
    const interval = setInterval(fetchDashboardData, 300000); // 5 minutes                  return () => clearInterval(interval);
  }, [fetchDashboardData]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center flex-col gap-8">                                                                                        <Loader2 className="h-16 w-16 text-blue-500 animate-spin" />
        <p className="text-xl text-gray-300">Loading your dashboard...</p>                    </div>                                                                                );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gray-950 flex items-center justify-center flex-col gap-6 text-center px-4">
        ...
        <button                                                                                   onClick={fetchDashboardData}                                                            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 rounded-xl text-white font-medium transition"
        >                                                                                         Retry
        </button>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-4 md:p-8 lg:p-12">                  {/* Attractive Disclaimer */}
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 md:gap-6">
          <LiveBitcoinPrice />
          {unreadMessages && (                                                                      <motion.button
              initial={{ scale: 0.95 }}
              animate={{ scale: 1 }}
              whileHover={{ scale: 1.05 }}
              onClick={() => navigate('/support')}
              className="flex items-center gap-3 bg-yellow-600/30 hover:bg-yellow-600/50 px-5 py-3 rounded-xl text-yellow-300 border border-yellow-600/40 transition-all"                   >                                                                                         <MessageSquare size={20} />
              <span>New Support Message</span>
            </motion.button>
          )}
          <button
            onClick={fetchDashboardData}                                                            className="p-3 bg-gray-800 hover:bg-gray-700 rounded-xl transition"
            title="Refresh Dashboard"
          >
            <RefreshCw size={22} />                                                               </button>
        </div>
      </header>                                                                         
      {/* Key Stats */}                                                                       <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 mb-10">
        <StatCard
          label="Total Portfolio Value"                                                           value={portfolio.totalValue}
          icon={Wallet}
          color="text-blue-400"
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-12">
        ...
        >
          <div className="flex items-center gap-6">
            <div className="bg-blue-500/20 p-6 rounded-2xl">                                          <TrendingUp size={36} className="text-blue-300" />
            </div>
            <div>
              <h3 className="text-2xl md:text-3xl font-bold mb-2">Trade Markets</h3>                  <p className="text-gray-300">Spot, futures & margin trading</p>                       </div>
          </div>
        ...
        >                                                                                         <div className="flex items-center gap-6">
            <div className="bg-gray-600/30 p-6 rounded-2xl">                                          <LifeBuoy size={36} className="text-gray-300" />
            </div>
            <div>
