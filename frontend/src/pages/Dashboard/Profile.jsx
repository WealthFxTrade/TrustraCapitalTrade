...
import api, { API_ENDPOINTS } from '../../api/api'; // Import API_ENDPOINTS
import { useAuth } from '../../context/AuthContext';

export default function Profile() {
  const navigate = useNavigate();
  const { logout, refreshAuth, isAuthenticated, initialized } = useAuth();
  ...
  useEffect(() => {
    const fetchProfile = async () => {                                      
      setLoading(true);
      try {
        // FIX: Use API_ENDPOINTS.USER.PROFILE instead of '/api/users/profile'
        // This prevents the "Double /api/api" 404 error
        ...
        toast.error('Identity Node connection failed');
      } finally {
        setLoading(false);
      }
    };                                                                  
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // FIX: Use API_ENDPOINTS.USER.PROFILE for the update as well
  ...
  if (!initialized || loading) {
    return (
      <div className="min-h-screen bg-[#020408] flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-gray-500 text-sm font-bold uppercase tracking-widest">Accessing Vault Identity...</p>
      </div>
    );
  }
  ...
  return (                                                               
    <div className="min-h-screen bg-[#020408] text-white flex font-sans overflow-hidden">
      ...
      {/* Main Content */}
      <main className="flex-1 p-6 lg:p-12 max-w-2xl mx-auto w-full overflow-y-auto">
        <div className="flex items-center gap-4 mb-8 lg:hidden">
          <button onClick={() => setMobileMenuOpen(true)} className="p-3 text-gray-400 hover:text-white">
            <Menu size={28} />
          </button>                                                             
