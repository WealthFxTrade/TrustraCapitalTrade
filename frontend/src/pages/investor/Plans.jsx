...
export default function Plans() {
  ...
  const getErrorMessage = (err) => {
    ...
    if (status === 400) {
      return 'Invalid request. Please check the amount or plan details.';
    }
    if (status === 403) {
      return 'You do not have permission to activate this plan.';
    }
    if (status === 409 || status === 422) {
      return 'This action cannot be completed (conflict detected).';
    }
    if (status >= 500) {
      return 'Service is temporarily unavailable. Please try again later.';
    }

    return err.message || 'An error occurred while activating the plan.';
  };
  ...
  const handleActivate = async (plan) => {
    ...
    if (isNaN(investAmount) || investAmount < plan.min) {
      return toast.error(`Minimum investment for \( {plan.name} is € \){plan.min}`);
    }

    if (stats.balances.EUR < investAmount) {
      return toast.error('Insufficient EUR balance. Please deposit or exchange funds first.');
    }
    ...
    <div className="p-8 bg-[#020617] min-h-screen text-white">
      <header className="mb-12 text-center sm:text-left">
        ...
        <h1 className="text-4xl font-black italic uppercase tracking-tighter">
          Rio <span className="text-slate-800">/</span> Investment Nodes
        </h1>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 max-w-7xl mx-auto">
        {PLAN_DATA.map((plan) => (
          <div
            key={plan.key}
            className={`bg-[#0f172a]/40 border border-white/5 p-8 rounded-[2.5rem] flex flex-col space-y-6 hover:border-indigo-500/30 transition-all backdrop-blur-xl relative overflow-hidden group`}
          >
            <div
              className={`absolute -top-10 -right-10 w-32 h-32 bg-\( {plan.color}-500/10 blur-[50px] rounded-full group-hover:bg- \){plan.color}-500/20 transition-all`}
            />
            ...
            <div>
              ...
              <p className="text-4xl font-black mt-2 tracking-tighter text-indigo-400">
                {plan.roi}
              </p>
            ...
            <button
              ...
              className={`w-full py-5 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] flex items-center justify-center gap-3 transition-all ${
                stats.balances.EUR >= plan.min && loading !== plan.key
                  ? 'bg-white text-black hover:bg-indigo-500 active:bg-indigo-600'
