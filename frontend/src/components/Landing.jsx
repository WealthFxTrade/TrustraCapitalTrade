...
export default function Landing() {
  const { price, loading } = useBtcPrice(300000); // 5-minute refresh
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col selection:bg-indigo-500/30">
      ...
      <nav className="border-b border-slate-800 bg-slate-900/80 sticky top-0 z-50 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              to="/register"
              className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 rounded-lg font-bold text-sm transition shadow-lg shadow-indigo-600/20"
            >
              Register
            </Link>
          </div>
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-20 text-center">
        ...
        <div className="flex flex-col sm:flex-row justify-center gap-4 mb-16">
          <Link
            to="/register"
            className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 rounded-xl font-bold transition flex items-center justify-center gap-2 shadow-xl shadow-indigo-600/10"
          >
            {landingHero.ctaPrimary} <ArrowRight className="h-5 w-5" />
          </Link>
          <a
            href="#plans"
            className="px-8 py-4 border border-slate-700 rounded-xl hover:bg-slate-900 transition font-bold"
          >
        ...
        <div className="mb-24 inline-block bg-slate-900/50 border border-slate-800 p-8 rounded-3xl backdrop-blur-sm">
          <p className="text-slate-500 uppercase tracking-widest text-[10px] font-bold mb-3">
          ...
          {loading && !price ? (
            ...
            <p className="text-4xl font-bold text-emerald-400 font-mono tracking-tighter">
              1 BTC = {price ? `$${price.toLocaleString()}` : '$77,494.65'}
            </p>
          )}
          <p className="text-[9px] text-slate-600 mt-2 italic">
            Real-time data provided by CoinGecko
          </p>
        </div>
        ...
        <section id="plans" className="grid md:grid-cols-2 lg:grid-cols-5 gap-6 scroll-mt-24 mb-32">
          {investmentPlans.map((plan) => (
            ...
            >
              ...
              <p className="text-slate-500 text-[10px] uppercase mb-6 tracking-widest font-bold">
                {plan.range}
              </p>
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-5 gap-6 mb-20">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className="bg-slate-900/20 border border-slate-800/50 p-6 rounded-2xl flex flex-col text-left hover:bg-slate-900/40 transition"
            >
              ...
              <div className="border-t border-slate-800/50 pt-4">
                <p className="text-[10px] font-bold text-indigo-400 tracking-wide uppercase">
                  {t.author}
                </p>
              </div>
            </div>
