...
/**
 * Trustra Capital Trade - Production Layout v8.4.1
 * Handles desktop navigation, mobile sticky-nav, and secure session display.
 */
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-white font-sans selection:bg-yellow-500/30">
      ...
      <nav className="bg-slate-900/40 border-b border-white/5 backdrop-blur-2xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            >
              <div className="w-10 h-10 bg-gradient-to-br from-yellow-400 to-yellow-600 rounded-xl flex items-center justify-center font-black text-black text-xl shadow-lg shadow-yellow-500/10 transform group-hover:scale-105 transition-transform">
                T
              </div>
            ...
            <div className="flex items-center gap-4">
              <div className="hidden sm:flex flex-col items-end border-r border-white/5 pr-4">
                ...
                <span className="text-[10px] font-bold text-yellow-500/80 font-mono">
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#020617]/90 backdrop-blur-3xl border-t border-white/5 px-8 py-4 z-50 flex justify-between items-center">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
            className={({ isActive }) =>
              `flex flex-col items-center gap-1 transition-all ${
                isActive ? 'text-yellow-500 scale-110' : 'text-slate-500'
              }`
            }
          >
            <item.icon size={20} />
      ...
      <footer className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 text-center">
        <div className="border-t border-white/5 pt-8">
          <p className="text-[9px] text-slate-700 uppercase font-black tracking-[0.4em] leading-relaxed">
            Trustra Capital Trade • Secure Audit Node v8.4.1 • Digital Assets Encrypted AES-256
          </p>
        </div>
      </footer>
    </div>
