...
const ledgerSchema = mongoose.Schema(
  {
    ...
    currency: { type: String, default: 'EUR', enum: ['EUR', 'BTC', 'ETH', 'USDT', 'ROI'] },
    type: { type: String, enum: ['deposit', 'withdrawal', 'yield', 'investment', 'transfer', 'referral_bonus', 'override'], required: [true, 'Transaction type is required'] },
...
const userSchema = mongoose.Schema(
  {
    ...
    email: { type: String, required: [true, 'Email address is required'], unique: true, trim: true, lowercase: true, match: [/^[^\s@]+@[^\s@]+\.[^\s@]+$/, 'Please provide a valid email'] },
    ...
    balances: {
      type: Map,
      of: Number,
      default: () => new Map([['EUR', 0], ['ROI', 0], ['BTC', 0], ['ETH', 0], ['USDT', 0], ['INVESTED', 0]]),
    },
