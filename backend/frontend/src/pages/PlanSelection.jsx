...
export default function PlanSelection() {
  ...
  const { token, setUser } = location.state || {};
  ...
  const handleSelectPlan = async () => {
    ...
    if (!selectedPlan || !numericAmount) {
      return setError('Please select a plan and enter a valid amount');
    }

    const plan = plans.find(p => p.name === selectedPlan);
    if (!plan) return setError('Selected plan is invalid');
    if (numericAmount < plan.min) return setError(`Minimum for ${plan.name} is $${plan.min}`);
    ...
    try {
      const res = await fetch(`${BACKEND_URL}/api/user/select-plan`, {
        ...
        body: JSON.stringify({ plan: selectedPlan, amount: numericAmount }),
      });

      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Plan selection failed');

      setUser(data.user); // Update Dashboard user state
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-12 text-indigo-400">
